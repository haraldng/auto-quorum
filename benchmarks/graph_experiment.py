...
def parse_clients_summaries(experiment_name: str) -> pd.DataFrame:
    ...
    experiment_data = [
        parse_client_summary(f) for f in experiment_directory.rglob("*client-1.json")
    ]
...
def graph_closed_loop_experiment(relative: bool = False, save: bool = True):
    # Get experiment data
    experiment_directory = "straggler-closed-loop-experiments-Opportunistic"
    ...
    if save:
        os.makedirs(save_path, exist_ok=True)
        plt.savefig(
            f"./logs/to_show2/{experiment_directory}/{run_directory}/latency_distribution.svg",
            format="svg",
        )
    plt.show()
    plt.close()
