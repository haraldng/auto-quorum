...
def closed_loop_experiment(cluster_size: int, total_messages: int, number_of_clients: int, persist_config: MetronomeCluster.PersistConfig=MetronomeCluster.PersistConfig.Individual()):
    experiment_log_dir = Path(f"./logs/closed-loop-experiments-batch/{cluster_size}-node-cluster-{number_of_clients}-clients")
    ...
    # Run experiments
    for data_size in [0, 4096, 1048*500]:
        for use_metronome in [0, 2]:
            ...
            experiment_log_prefix = f"metronome-{use_metronome}-datasize-{data_size}"
...
def closed_loop_experiment_sleep(cluster_size: int, total_messages: int, number_of_clients: int):
    experiment_log_dir = Path(f"./logs/closed-loop-experiments-sleep/{cluster_size}-node-cluster-{number_of_clients}-clients")
    ...
    # Run experiments
    for storage_delay in [0, 500, 1000, 2000]:
        for use_metronome in [0, 2]:
            ...
            cluster.await_client(1)
            cluster.await_servers()
            experiment_log_prefix = f"metronome-{use_metronome}-delay-{storage_delay}"
            cluster.get_logs(experiment_log_dir, experiment_log_prefix)
    # cluster.shutdown()
...
def main():
    closed_loop_experiment(5, 3000, 30)
    # closed_loop_experiment_sleep(5, 1000, 10)
    # metronome_size_experiment(7, 1000, 1)
    pass
